        ...
        <tr>
            ...
            <td>
                <div class="button-container">
                    ...
                    <button id="btnEditNV" class="btn btn-danger" onclick="deleteNhanVien('${item.tknv}')">Delete</button>
                </div>
            </td>
        </tr>
        `;
...
window.editNhanVien = (tknv) =>{
    nhanVienToEdit = nhanVienServiceInstance.danhSachNhanVien.find(item.tknv === tknv);
    if(nhanVienToEdit){
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnEditNV').onclick = ()=>{
        console.log('Đang bấm vào nút lưu chỉnh sửa');
        const nhanVien = layThongTinNhanVien();
        if(nhanVien){
            // Cập nhật nhân viên
            nhanVienServiceInstance.
        }
    }
});
